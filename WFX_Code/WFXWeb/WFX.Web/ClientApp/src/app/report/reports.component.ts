    ...
    this.excelexportservice.DefectAnalysis(this.ordermodel).subscribe(
      res => {
        ...
        if (!worksheet.columns) {
          ...
          var cols: any = {};
          const fixColIndexLength = Object.keys(this.data_defect[0]).indexOf('DefectsFound');
          for (let i = 0; i <= fixColIndexLength; i++) {
            cols[Object.keys(this.data_defect[0])[i]] = '';
          }

          this.data_defect.forEach(x => {
            for (let i = fixColIndexLength+1; i < Object.keys(x).length; i++) {
              if (!cols.hasOwnProperty(Object.keys(x)[i])) {
                cols[Object.keys(x)[i]] = '';
              }
            }
          });

          this.data_defect.forEach(x => {
            for (let i = fixColIndexLength+1; i < Object.keys(cols).length; i++) {
              if (!x.hasOwnProperty(Object.keys(cols)[i])) {
                x[Object.keys(cols)[i]] = 0;
              }
            }
          });
          worksheet.columns = Object.keys(cols).map((k) => ({ header: k, key: k }));
        }
        this.data_defect.forEach(x => {
          worksheet.addRow(x);
        });
