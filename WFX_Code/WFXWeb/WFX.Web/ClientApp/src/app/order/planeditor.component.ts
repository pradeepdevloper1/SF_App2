...
export class PlanEditorComponent implements OnInit {
  ...
  ngOnInit() {
    this._Activatedroute.paramMap.subscribe(params => {
      ...
      this.planmodel.sono = params.get('sono');
  ...
  SampleFile() {
      ...
      this.http.post<any>(endpoint, formData)
        .subscribe(res => {
          if (res.status = 200) {
            ...
            this.router.navigate(['/User/TargetUpload/',this.planmodel.sono]);
          }
        });
    }
   ...
   async savechanges() {
    ...
    onPOclick(){
      this.router.navigate(['/User/POView/' + this.planmodel.sono], { queryParams: { calledFrom: 'ProductionOrder' } });
