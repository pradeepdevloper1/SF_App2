...
export class UploadOBService {
  ...
  GetOBFileUploadList(opt: { ProductID: number, ProcessTypeCode: string }) {
  ...
  GetOBProducts(processTypeCode: string) {
    var res = this.http.get<any>(this.Url + "UploadOB/GetOBProducts?ProcessTypeCode=" + processTypeCode, {
      headers: new HttpHeaders().set('Authorization', "Bearer " + this.auth)
    });
    return res;
  }
