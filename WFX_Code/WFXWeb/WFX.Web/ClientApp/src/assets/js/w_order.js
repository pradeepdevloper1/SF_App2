...
var gridOptions = {
  columnDefs: [
    ...
    { field: "ProcessCode", minWidth: 100 },
    { field: "ProcessName", minWidth: 100 },
...
function getlist() {
  ...
  if (parseInt(errorcount) == 0) {
    ...
    if(res.status != 200){
      ...
      oneerror = "<tr><th>Row 1 </th> <td> " + res.message + " .</td></tr>";
...
function geterrorcount() {
  ...
  gridOptions.api.forEachNode((rowNode, index) => {
    ...
    if (rowNode.data.ProcessName == null || rowNode.data.ProcessName == "") {
      errorcount = errorcount + 1;
      oneerror = "<tr><th>Row " + rowno + "</th> <td>Please fill Process Name.</td></tr>";
      $("#errortable").append(oneerror);
    }
    if (rowNode.data.ProcessCode == null || rowNode.data.ProcessCode == "") {
      errorcount = errorcount + 1;
      oneerror = "<tr><th>Row " + rowno + "</th> <td>Please fill Process Code.</td></tr>";
      $("#errortable").append(oneerror);
    }
...
var apiurl = sessionStorage.getItem("apiurl") + 'Order/PostIsProcessCodeExist';
$.ajax({
  cache: false,
  url: apiurl,
  headers: { 'Authorization': "Bearer " + sessionStorage.getItem('auth') },
...
if (res.status == 401) {
  errorcount = errorcount + 1;
  oneerror = "<tr><th>Row " + rowno + " </th> <td>Process Code doesn’t exist in the factory.</td></tr>";
  $("#errortable").append(oneerror);
}
if (res.status == 200 && res.data) {
  if(res.data.processCode !==rowNode.data.ProcessCode){
    errorcount = errorcount + 1;
    oneerror = "<tr><th>Row " + rowno + " </th> <td>Process Code doesn’t exist in the factory.</td></tr>";
    $("#errortable").append(oneerror);
    }
  if(res.data.processName !==rowNode.data.ProcessName && rowNode.data.ProcessName !==null &&  rowNode.data.ProcessName !== '' && res.data.processCode ==rowNode.data.ProcessCode && rowNode.data.ProcessName){
  errorcount = errorcount + 1;
  oneerror = "<tr><th>Row " + rowno + " </th> <td>Process Name is invalid for the particular Process Code.</td></tr>";
  $("#errortable").append(oneerror);
  }
}
}
    ...
    gridOptions.api.forEachNode((rowNode, index) => {
      ...
      if (value.PONo == rowNode.data.PONo) {
        ...
        if (firstpoindex == 0) {
          ...
          if (Color == rowNode.data.Color && PONo == rowNode.data.PONo && Part == rowNode.data.Part && ProcessName == rowNode.data.ProcessName) {
            errorcount = errorcount + 1;
            oneerror = "<tr><th>Row " + rowno + "</th> <td>Duplicate records not allowed for same PO, Part,Color and ProcessName </td></tr>";
            $("#errortable").append(oneerror);
          }
        }
      }
