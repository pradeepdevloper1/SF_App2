...
namespace XF.APP.DAL
{
    public class PurchaseOrderRepository : BaseRepository, IPurchaseOrderRepository
    {
        ...
        public async Task<IEnumerable<PurchaseOrderDto>> GetByIdPoAsync(string soNo)
        {
            _dbContext = new ApplicationContext(Constants.DbPath);
            var model = await _dbContext.PurchaseOrder.Where(p => p.SoNo == soNo).ToListAsync();
            ...
            foreach (var po in modelDTO)
            {
                ...
                if (modelList.Count > 0)
                {
                    ...
                    if (modellist2.Count > 0)
                    {
                        ...
                        modellist2[0].StyleRef = po.StyleRef;
                        modellist2[0].SizeList = po.SizeList;
                        modellist2[0].TotalColors = po.TotalColors;
                        modellist2[0].CompletedColors = po.CompletedColors;
                        modellist2[0].CompletedQty = po.CompletedQty;
                        modellist2[0].ColorsGraphProgress = po.ColorsGraphProgress;
                        modellist2[0].QtyGraphProgress = po.QtyGraphProgress;
